...
module.exports = {
    ...
    run: async (client, message, args, prefix) => {
        ...
        if (!usuario) return message.reply(`<:VS_cancel:1006609599199186974> **No se ha encontrado al usuario que has especificado!**`);
        ...
        if (usuario.id == message.guild.ownerId) return message.reply(`<:VS_cancel:1006609599199186974> **No puedes expulsar al DUEÑO del Servidor!**`);
            ...
            if (message.member.roles.highest.position > usuario.roles.highest.position) {
                ...
                usuario.kick([razon]).catch(() => {
                    return message.reply({
                        embeds:
                            [new Discord.EmbedBuilder()
                                .setTitle(`<:VS_cancel:1006609599199186974> No he podido banear al usuario!`)
                                .setColor("FF0000")
                            ]
                    })
                })
            } else {
                return message.reply(`<:VS_cancel:1006609599199186974> **Tu Rol está por __debajo__ del usuario que quieres banear!**`)
            }
        } else {
            return message.reply(`<:VS_cancel:1006609599199186974> **Mi Rol está por __debajo__ del usuario que quieres banear!**`)
        }

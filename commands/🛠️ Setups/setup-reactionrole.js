...
module.exports = {
    ...
    run: async (client, message, args, prefix) => {
        ...
        async function emoji(){
            ...
            preguntar.awaitReactions({
                ...
                if(collected.first().emoji.id && collected.first().emoji.id.length > 2){
                    ...
                    message.reply(`<:VS_cancel:1006609599199186974> Cancelado y finalizando setup...`)
                    return finalizar();
                }
            }).catch(() => {
                return finalizar();
            ...
            async function rol(){
                ...
                await querol.channel.awaitMessages({
                    ...
                    if(rol) {
                        ...
                        return message.reply(`<:VS_cancel:1006609599199186974> **El Rol que has mencionado NO EXISTE!**\nSetup cancelado!`)
                    }
                }).catch(() => {
                    return finalizar();
                })
        ...
        async function finalizar() {
            if(contador === 1 && !objeto.Parametros.length) return message.reply(`<:VS_cancel:1006609599199186974> **Tienes que añadir al menos un emoji con un rol**\nSetup cancelado!`);
                ...
                if(canal) {
                    ...
                    await idmensaje.channel.awaitMessages({
                        ...
                        if(encontrado){
                            ...
                            return message.reply(`<:VS_cancel:1006609599199186974> **No se ha encontrado el mensaje que has especificado!**\nSetup cancelado!`)
                        }
                    }).catch((e) => {
                        return message.reply(`<:VS_cancel:1006609599199186974> Tu tiempo ha expirado!`)
                    })
                } else {

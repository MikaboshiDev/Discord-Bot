    ...
    client.on("interactionCreate", async interaction => {
        try {
            ...
            for (const ticket of ticket_data) {
                if (interaction.guild.channels.cache.get(ticket.canal)) return interaction.reply({ content: `<:VS_cancel:1006609599199186974> **Ya tienes un ticket creado en <#${ticket.canal}>**`, ephemeral: true });
            }
            ...
            channel.send({
                embeds: [new Discord.EmbedBuilder()
                    ...
                    .setDescription(`📣 Bienvenido al soporte del servidor ${interaction.member}`) 
                    .addFields(
         {name:`🖥️ Soporte Sistema de Tickets`, value:`Explica detalladamente tu problema actual y un staff del equipo pronto tratara de ayudarte`},
         {name:`➥・User`, value:`${interaction.member}`, inline: true},
         {name:`➥・Sistema`, value:`Tickets Servidor`, inline: true},
         {name:`➥・Canal`, value:`${channel}`, inline: true},
         {name:`🎲 Important Annoument`, value: "```" + `Recuerda que abrir un ticket de forma de burla o para molestar es sancionable con aislamiento o baneo permanente` + "```"})
        ...
        try {
            ...
            switch (interaction.customId) {
                case "cerrar_ticket":{
                    ...
                    if(ticket_data && ticket_data.cerrado) return interaction.reply({content: `<:VS_cancel:1006609599199186974> **Este ticket ya estaba cerrado!**`, ephemeral: true});
